...
$error["Nombres"]="El nombre no es válido";
  ...
  if(!empty($_POST["Apellidos"])&& !is_numeric($_POST["Apellidos"]) && !preg_match("/[0-9]/", $_POST["Apellidos"])){
    ...
    if(count($error)==0){
      $sql= "INSERT INTO estudiante VALUES(NULL, '{$_POST["Nidentificacion"]}', '{$_POST["Nombres"]}', '{$_POST["Apellidos"]}', '{$_POST["Direccion"]}', '{$_POST["Acudiente"]}', '{$_POST["TelefonoA"]}', '".sha1($_POST["Password"])."', '{$_POST["Rol"]}', '{$image}');";
